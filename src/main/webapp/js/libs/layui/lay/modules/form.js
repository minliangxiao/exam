...
layui.define('layer', function(exports){
    ...
    this.config = {
      verify: {
        ...
        ,number: function(value,element){
            ...
            var minVal = $(element).attr("min");
            //最大值验证
            if (maxVal && !isNaN(value)) {
                if (parseFloat(value) > parseFloat(maxVal)) {
                    $(element).val(maxVal);
                }
            }
            //最小值验证
            if (minVal && !isNaN(value)) {
        ...
        regular: function (value, element) {
            var exp = new RegExp($(element).attr("expression"));
            if (!exp.test(value)) {
                return '验证没通过'
            }
        }
  ...
  Form.prototype.valid = function (isShowTips) {
      
      var verifyElem = $("[lay-verify]:not([disabled])");
      var errCount = 0;
      isShowTips = isShowTips == undefined ? true : isShowTips;
      var tips = [];
      verifyElem.each(function () {
          var tip
          if (tip = elemValid(this, isShowTips))
              tips.push(tip);
      });
  ...
  dom.on('submit', ELEM, submit).on('click', '*[lay-submit]', submit);
    ...
    isShowTips:是否弹出层提示
    */
  var elemValid = function (elem, isShowTips) {
      var othis = $(elem), ver = othis.attr('lay-verify').split('|'), verify = form.config.verify;
      var isPass = true, value = othis.val();
      var fieldName = othis.attr("lay-fieldName") || '', tips = '';
      var tipIndex = othis.attr('tipIndex');
      for (var i = 0, len = ver.length; i < len; i++) {
          ...
          if (verify[thisVer] && (isFn ? tips = verify[thisVer](value, othis) : !verify[thisVer][0].test(value))) {
              ...
              if (isShowTips) {
                  ...
                  othis.attr('tipIndex', layer.tips(tips, othis, { tips: 1, tipsMore: true,time:3000 }));
              }
              isPass = false;
              break;
          }
      ...
      if (!tips) {
          if (tipIndex)
            layer.close(tipIndex);
          othis.removeClass("required");
      } else {
          
          othis.addClass("required");
      }
      return tips;
  }
  ...
  $(document).ready(function () {
      ...
      verifyElem.on("change", function () {
          elemValid(this, isShowTips);
      });
      verifyElem.change();
      isShowTips = true;
