                ...
                (function () {
                    app = new Vue({
                        ...
                        data: {
                            ...
                            //{ title: '试卷', render: (h, params) => { return params.row.testpaper.name } },
                            { title: '名称', key: 'name' },
//                            { title: '开始时间', render: (h, params) => { return renderDate(h, params.row.startTime); } },
//                            { title: '【totalStudentQty】', render: (h, params) => { return renderNumberWithThousands(h, params.row.totalStudentQty); } },
//                            { title: '【examStudentQty】', render: (h, params) => { return renderNumberWithThousands(h, params.row.examStudentQty); } },
//                            { title: '【endTime】', render: (h, params) => { return renderDate(h, params.row.endTime); } },
//                            { title: '【finishStudentQty】', render: (h, params) => { return renderNumberWithThousands(h, params.row.finishStudentQty); } },
//                            { title: '状态', key: 'status' },
                            { title: '总分', render: (h, params) => { return renderNumberWithThousands(h, params.row.totalScore); } },
//                            { title: '【avgScore】', render: (h, params) => { return renderNumberWithThousands(h, params.row.avgScore); } },
//                            { title: '类型', key: 'type' },
//                            { title: '【publicAnswer】', key: 'publicAnswer' },
                            {
                    ...
                    methods: {
                        ...
                        displayItem: function (item) {
                            var me = this;
                            layer.confirm('报考后将无法取消，确认要报考吗？', { icon: 2, title: '提示' }, function (index) {
                                common.callApi({
                                    url: "exam/register"
                                    , data: { id: item.id }
                                    , cb: function (r) {
                        ...
                        publish: function (item) {
                            var me = this;
                            layer.confirm('发布后将无法取消，确认要发布吗？', { icon: 2, title: '提示' }, function (index) {
                                common.callApi({
                                    url: "exam/publish"
                                    , data: { id: item.id }
                                    , cb: function (r) {
                        ...
                        publishScore: function (item) {
                            var me = this;
                            layer.confirm('公布后将无法取消，确认要公布成绩吗？', { icon: 2, title: '提示' }, function (index) {
                                common.callApi({
                                    url: "exam/publishScore"
                                    , data: { id: item.id, publishAnswer:'否' }
                                    , cb: function (r) {
                                        me.getData();
                                        me.$Message.success('操作成功');
                                    }
                        ...
                        publishAnswer: function (item) {
                            var me = this;
                            layer.confirm('公布后将无法取消，确认要公布成绩和答案吗？', { icon: 2, title: '提示' }, function (index) {
                                common.callApi({
                                    url: "exam/publishScore"
                                    , data: { id: item.id, publishAnswer:'是' }
                                    , cb: function (r) {
                                        me.getData();
                                        me.$Message.success('操作成功');
                                    }
