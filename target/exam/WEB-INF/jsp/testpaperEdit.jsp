        ...
        <tabs>
            <tab-pane label="试卷信息">
                <Form-item label="年级" prop="gradeId">
                    <i-select v-model="modelData.gradeId" placeholder="请选择年级" filterable="true" @on-change="gradeChange">
                ...
                <Form-item label="试题类型" prop="subjectId">
                    <i-select v-model="modelData.subjectId" placeholder="请选择试题类型"  @on-change="subjectChange">
                ...
                <Row>
                		<i-col span="16">
		                    <i-select v-model="questionId" placeholder="请选择试题" filterable>
		                        <i-option v-for="item in questionIdSuggestDataList" :value="item.id" :key="item.id">{{ item.name }}</i-option>
		                    </i-select>                		
                		</i-col>
                		<i-col span="6"><i-input v-model="score" placeholder="分数"></i-input></i-col>
                		<i-col span="2"><i-button type="primary" v-on:click="addItem">添加</i-button></i-col>
                </Row>
    ...
    (function () {
        ...
        questionId:null,
            ...
            subjectChange:function(v){
                ...
                getData: function () {
                    ...
                    if(entityId>0) {
                        ...
                        getSubjectIdSuggestDataList: function () {
                            ...
                            common.callApi({
                                url: "subject/suggest"
                                , data: { keyword: "", gradeId:me.modelData.gradeId }
                                , cb: function (r) {
                       	...
                       	getQuestionIdSuggestDataList: function () {
                            ...
                            common.callApi({
                                ...
                                , data: { keyword: "", subjectId: me.modelData.subjectId, gradeId:me.modelData.gradeId }
                                , cb: function (r) {
                ...
                saveData: function () {
                    var me = this;
                    if(me.modelData.totalScore!=100) {
                    	me.$Message.error('总分数必须是100分。');
                    	return;
                    }
                    // UEditor 用以下代码
                    ...
                    this.$refs.modelData.validate((valid) => {
                        if (valid) {
                            ...
                            for(i=0; i < me.addedList.length; i++) {
                            	added.push(me.addedList[i]);
                            }
                            for(i=0; i < me.scoreList.length; i++) {
                            	scores.push(me.scoreList[i]);
                            }
                            for(i=0; i < me.removedList.length; i++) {
                            	if(me.removedList[i]>=1){
	                            	deleted.push(me.removedList[i]);
                            	}                            
                            }
                            me.modelData.added = added.join(",");
                            me.modelData.deleted = deleted.join(",");
                            me.modelData.scores=scores.join(",");
                            common.callApi({
                                url: "testpaper/save", data: me.modelData,
                                cb: function (r) {
                                    me.postIng = false;
