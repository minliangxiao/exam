...
<html>
    <head>
		...
		<base href="<%=basePath%>">  
        <title>编辑【QuestionInTestpaper】信息</title>
...
<div style="padding: 10px;" id="app">
    <i-form ref="modelData" v-bind:model="modelData" v-bind:rules="ruleValidate" label-width="80">
        <tabs>
            <tab-pane label="【QuestionInTestpaper】信息">
                <Form-item label="【testpaperId】" prop="testpaperId">
                    <i-select v-model="modelData.testpaperId" placeholder="请选择【testpaperId】">
                        <i-option v-for="item in testpaperIdSuggestDataList" :value="item.id" :key="item.id">{{ item.name }}</i-option>
                    </i-select>
                </Form-item>
                <Form-item label="试题" prop="questionId">
                    <i-select v-model="modelData.questionId" placeholder="请选择试题">
                ...
                saveData: function () {
                    ...
                    this.$refs.modelData.validate((valid) => {
                        if (valid) {
                            ...
                            common.callApi({
                                url: "questionInTestpaper/save", data: me.modelData,
                                cb: function (r) {
                                    me.postIng = false;
